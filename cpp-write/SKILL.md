...
**L0 PR-body check** (only when the task includes opening or updating a PR): does the project ship a PR template at `.github/pull_request_template.md`, `.github/PULL_REQUEST_TEMPLATE/*.md`, or a module-local equivalent? If so, the PR body **must** use the template's section headings verbatim -- do not author a free-form summary. Read the template before drafting the body; replace every italic placeholder with content drawn from the diff and the session; populate Risk / Halo / Agentic-AI sections explicitly (a best-guess flagged for confirmation is acceptable; an omission is not).

**L0 PR-body voice -- the "elevator pitch" rule.** The Purpose / equivalent first section is an elevator pitch in natural English, not a catalogue. Anchor on the *problem the change solves* and the *value it adds for someone working in this area*. Suppress: bullet lists of new types/files, **catalogue code-snippet tours**, stacking/PR-chain references, recitation of threading/lifetime/layering contracts, out-of-scope enumerations beyond a single sentence, and any phrase that pivots the section into a deliverables list ("what this PR adds", "what this PR does"). One short exemplar code block showing the new consumer surface in use is acceptable when the API shape is non-obvious from prose -- one block, five to fifteen lines, realistic names, consumer side only. The diff itself shows what was added; the body has to explain why it matters.

**The same discipline applies throughout the body, not only the Purpose section.** Every section has a default-tight shape: *Functional Testing* is a suite name + count + one sentence on what is pinned (never a table of test names); *Performance Testing* is one sentence (skip the lecture); *Risk / Halo* is a number plus a one-sentence justification (never `Reviewer guess: confirm before publishing` left in); *Comments to reviewers* is one to three bullets greeting the reviewer ("look at X first because Y") -- never process narrative ("`cpp-review` ran, N findings applied, M deferred"), never references to specific Doxygen sections, never re-stated contracts that already live in code comments, never base-branch / auto-rebase notes. If a bullet would not teach the reviewer something they could not learn by opening the file, drop it.

If the diff introduces a framing block at the top of a new file (the `@brief` on the principal new class, or a namespace-level docblock), that prose *is* the elevator pitch -- reuse it. The cheapest finding to fix is the one that doesn't ship.
